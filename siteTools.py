...
def getCiteInfo(title, cookie, debug=False):
    ...
    firstindex_begin = result.find('<div class=\"gs_r\"')
    firstindex_end = result.find('<div class=\"gs_r\"', firstindex_begin+1)
    ...
    citeidinfo = paperinfo[citeid_begin:citeid_end-1].split(',')
    if(debug):
        print citeidinfo
    if(len(citeidinfo) <= 2):
        print 'the citeid rule shoule be updated.'; exit()
    citeid = citeidinfo[1][1:-1]
    if(debug):
        print 'citeid: ' + citeid
